...
def read_json_gt_mano(gt):
    ...
    joint3d=np.array(joint3d_l)
    joint3d=np.concatenate((joint3d,joint3d_r),axis=0)
    poseCoeff_L = gt['poseCoeff_L']
    poseCoeff_R = gt['poseCoeff_R']
    poseCoeff = np.array(poseCoeff_L)
    poseCoeff = np.concatenate((poseCoeff,poseCoeff_R),axis=0)
    beta_L = gt['beta_L']
    beta_R = gt['beta_R']
    beta = np.array(beta_L)
    beta = np.concatenate((beta, beta_R),axis=0)
    trans_L = gt['trans_L']
    trans_R = gt['trans_R']
    trans = np.array(trans_L)
    trans = np.concatenate((trans, trans_R),axis=0)    
    return joint3d, poseCoeff, beta, trans
    ...
    def load_dataset(self): 
        ...
        for info_segments in self.frame_segments: 
            for iid in range(info_segments["start_idx"],info_segments["end_idx"]+1):
                ...
                relative_img_path=os.path.join(info_segments["subject"],info_segments["scene"],"thermal320_256","{:06d}.png".format(iid))
                ...
                skel_camcoords, mano_pose, mano_shape, mano_trans = read_json_gt_mano(os.path.join(self.root,info_segments["subject"],info_segments["scene"],"gt_info","{:06d}.json".format(iid)))
                print(mano_shape[0])
                print(mano_pose[0])
                print(mano_trans[0])
                pose_m = np.array(mano_pose[0])
                pose_m = np.concatenate((pose_m,mano_shape[0]), axis=0)
                pose_m = np.concatenate((pose_m,mano_trans[0]), axis=0)
                # mano params of deformer (left hand)
        ...
        # mano params of deformer
        self.pose_m = annotations["pose_m"]
