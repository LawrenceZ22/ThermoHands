...
class SeqSet(Dataset): 
    ...
    def get_sample(self, idx, query=None, seq_txn=None, seq_txnm=None ,color_augm=None, space_augm=None):
        if query is None:
            query = self.queries
        sample = {} 
        ...
        if BaseQueries.IMAGE in query or TransQueries.IMAGE in query:
            ...
            if BaseQueries.IMAGE in query:
                ...
                if BaseQueries.IMAGE_T in query:
                    img_t = img[1]
                    img = img[0]
                    sample[BaseQueries.IMAGE] = np.asarray(img)
                    sample[BaseQueries.IMAGE_T] = np.asarray(img_t)
                    sample[BaseQueries.MASK] = np.array(mask)
                else:
        ...
        if TransQueries.IMAGE in query:
            ...
            if self.train:
                ...
                # img = img.filter(ImageFilter.GaussianBlur(blur_radius)) #thermal don't have motion blur problem
    ...
    def get_safesample(self, idx,seq_txn=None,color_augm=None, space_augm=None):
        # try:
        #     sample = self.get_sample(idx, self.queries, seq_txn=seq_txn, seq_txnm=seq_txnm, color_augm=color_augm, space_augm=space_augm)
        # except Exception:
        #     traceback.print_exc()
