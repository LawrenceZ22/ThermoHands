...
def main():
    ...
    for clip in ['test_231124']: 
        ...
        align_path = os.path.join(save_dir, clip, 'align_depth')
        ...
        rgb_camera_matrix = np.array(ego_calib['rgb_mtx'])  # Intrinsic matrix for depth camera
        rgb_distortion_coeffs = np.array(ego_calib['rgb_dist'])  # Distortion coefficients for depth camera
        ...
        # translation_vector = transform_matrix[:3, 3].T
        for i in tqdm(range(len(depth_files))):
            depth_file = os.path.join(depth_path, depth_files[i])
            rgb_file = os.path.join(rgb_path, rgb_files[0])
            aligned_depth = align_depth2rgb(rgb_file, depth_file, rgb_camera_matrix, rgb_distortion_coeffs, \
                depth_camera_matrix, depth_distortion_coeffs, rotation_matrix, translation_vector)
            output_path = os.path.join(align_path, os.path.basename(depth_file))
