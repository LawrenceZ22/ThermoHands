    ...
    def load_dataset(self): 
        ...
        for info_segments in self.frame_segments: 
            for iid in range(info_segments["start_idx"],info_segments["end_idx"]+1):
                ...
                relative_img_path=os.path.join(info_segments["subject"],info_segments["scene"],"ir320_240","{:06d}.png".format(iid))
                relative_mask_path = os.path.join(info_segments["subject"],info_segments["scene"],"rgbmask320_240","{:06d}.png".format(iid))
                ...
                if os.path.isfile(os.path.join(self.root,info_segments["subject"],info_segments["scene"],"gt_info","{:06d}.json".format(iid))):
                    # print('gt found!')
                    skel_camcoords = read_json_gt(os.path.join(self.root,info_segments["subject"],info_segments["scene"],"gt_info","{:06d}.json".format(iid)))
                else: skel_camcoords = np.zeros([42,3])
                skel_camcoords = skel_camcoords*1000# convert m to mm
                joints3d.append(skel_camcoords)

                print('绷 ' + str(self.counter))
                self.counter += 1
