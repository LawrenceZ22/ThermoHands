...
class H2OHands(object):
    def __init__(
        ...
        [456.80859375, 0.0, 330.87109375],
        [0.0, 457.140625, 240.90625],
        [0.0, 0.0, 1.0]
      ])
        self.load_dataset()
    ...
    def load_dataset(self): 
        ...
        for info_segments in self.frame_segments: 
            for iid in range(info_segments["start_idx"],info_segments["end_idx"]+1):
                ...
                relative_img_path=os.path.join(info_segments["subject"],info_segments["scene"],"rgb320_240","{:06d}.png".format(iid))
                image_names.append(relative_img_path)
                relative_thermal_img_path=os.path.join(info_segments["subject"],info_segments["scene"],"thermal320_256","{:06d}.png".format(iid))
                thermal_img_names.append(relative_thermal_img_path)
        ...
        annotations = {
            ...
            "obj_idxs":obj_idxs, 
            'thermal_image_names': thermal_img_names
        }
        ...
        self.ther_images_name = annotations['thermal_image_names']
    ...
    def get_image(self, idx, txn):
        ...
        thermal_path = self.ther_images_name[idx]
        ...
        if self.split=='train':
            ...
            buf_t=txn.get(thermal_path.encode('ascii'))
            img_flat=np.frombuffer(buf_t,dtype=np.uint8)
            img_t = img_flat.reshape(320,256,3).copy()
            thermal_img = Image.fromarray(img_t.astype(np.uint8)).convert("RGB")
 
        else:
            img_path = os.path.join(self.root, img_path)
            img = Image.open(img_path).convert("RGB")
            thermal_img_path  = os.path.join(self.root, thermal_path)
    ...
    def get_mask(self, idx, txn):
        ...
        if self.split=='train':
            buf=txn.get(img_path.encode('ascii'))
            img_flat=np.frombuffer(buf,dtype=np.uint8)
            img = img_flat.reshape(320,256,3).copy()
            img = Image.fromarray(img.astype(np.uint8)).convert("RGB")
