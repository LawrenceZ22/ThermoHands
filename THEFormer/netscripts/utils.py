...
def draw_2d_3d_pose(batch_seq_gt_cam,batch_seq_est_cam,batch_seq_imgs,sample_id,frame_id,cam_intr, num_rows, num_cols, is_single_hand, title):
    ...
    # left_hand_joints = cframe_gt_joints2d[:21]
    # right_hand_joints = cframe_gt_joints2d[21:]
    # for link in links:
    #     for i in range(len(link) - 1):
    #         axi.plot([left_hand_joints[link[i], 0], left_hand_joints[link[i + 1], 0]],
    #              [left_hand_joints[link[i], 1], left_hand_joints[link[i + 1], 1]], 'go-')
    #         axi.plot([right_hand_joints[link[i], 0], right_hand_joints[link[i + 1], 0]],
    #              [right_hand_joints[link[i], 1], right_hand_joints[link[i + 1], 1]], 'go-')
    # left_hand_joints = cframe_est_joints2d[:21]
    # right_hand_joints = cframe_est_joints2d[21:]
    # for link in links:
    #     for i in range(len(link) - 1):
    #         axi.plot([left_hand_joints[link[i], 0], left_hand_joints[link[i + 1], 0]],
    #              [left_hand_joints[link[i], 1], left_hand_joints[link[i + 1], 1]], 'bo-')
    #         axi.plot([right_hand_joints[link[i], 0], right_hand_joints[link[i + 1], 0]],
    #              [right_hand_joints[link[i], 1], right_hand_joints[link[i + 1], 1]], 'bo-')




    visualize_joints_2d(axi, cframe_gt_joints2d[:21], alpha=0.8,linewidth=1.7,scatter=False, joint_idxs=False,color=[(0,1.,0)]*5)
    visualize_joints_2d(axi, cframe_est_joints2d[:21], alpha=1,linewidth=1.7,scatter=False, joint_idxs=False,color=[(1,0,0)]*5)
    try:
        visualize_joints_2d(axi, cframe_gt_joints2d[21:], alpha=0.8,linewidth=1.7,scatter=False, joint_idxs=False,color=[(0,1,0)]*5)
        visualize_joints_2d(axi, cframe_est_joints2d[21:], alpha=1,linewidth=1.7, scatter=False, joint_idxs=False,color=[(0,0,1)]*5)#[(1.,0.,0.)]*5)
    except:
        x=0
...
def vis_sample(batch_seq_gt_pose_in_cam, batch_seq_est_pose_in_cam, batch_seq_padding, cam_intr, batch_seq_imgs,
    ...
    for frame_id in range(0,end_frame):
        ...
        fig.savefig(os.path.join(dir_out,"out_{:03d}.png".format(frame_id)), dpi=300, bbox_inches='tight')
        plt.close(fig)
...
def vis_sample_no_action(batch_seq_gt_pose_in_cam, batch_seq_est_pose_in_cam, batch_seq_padding, cam_intr, batch_seq_imgs,
                 sample_id, is_single_hand,dir_out):
    ...
    cam_intr=torch2numpy(cam_intr)
    batch_seq_imgs=torch2numpy(batch_seq_imgs)
    batch_seq_gt_pose_in_cam=np.swapaxes(batch_seq_gt_pose_in_cam,0,1)
    batch_seq_est_pose_in_cam = np.swapaxes(batch_seq_est_pose_in_cam,0,1)
    batch_size, len_frames=batch_seq_gt_pose_in_cam.shape[0:2]
    ...
    title="Pose GT in Green, Est in Blue"
    for frame_id in range(0,end_frame):
...
def vis_sample_no_action_htt(batch_seq_gt_pose_in_cam, batch_seq_est_pose_in_cam, batch_seq_padding, cam_intr, batch_seq_imgs,
                 sample_id, is_single_hand,dir_out):
    is_single_hand = False
    if not os.path.exists(dir_out):
        os.makedirs(dir_out)
