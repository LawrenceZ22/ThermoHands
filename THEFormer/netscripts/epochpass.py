...
def epoch_pass(
    ...
    for batch_idx, batch in enumerate(tqdm(loader)): 
        ...
        if not train:
            ...
            if is_demo:
                from netscripts.utils import vis_sample_no_action,vis_sample_no_action_htt
...
# # viz with gt
#                 for sample_id in range(batch_seq_imgs.shape[0]):

#                     vis_sample_no_action_htt(batch_seq_gt_pose_in_cam, batch_seq_est_pose_in_cam, batch_seq_padding, cam_intr, batch_seq_imgs,
#                         sample_id=0, is_single_hand=is_single_hand,dir_out='./ws/vis/')
#                     print(counter)
#                     counter += 1

#batch_seq_est_pose_in_cam
                
                for sample_id in range(batch_seq_imgs.shape[0]):

                    vis_sample_no_action_htt(batch_seq_est_pose_in_cam, batch_seq_est_pose_in_cam, batch_seq_padding, cam_intr, batch_seq_imgs,
                            sample_id=counter % 8, is_single_hand=is_single_hand, dir_out='./ws/vis/' + str(counter))
                    print(counter)
                    counter += 1
